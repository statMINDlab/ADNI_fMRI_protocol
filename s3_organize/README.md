...
**3.10.)** Once you have finalized the list of DICOM directories, you can use `make_subject_list_for_conversions.sh` to create the list of subjects to pass on to the next step, where we convert to NIfTI and BIDS-ify the data using Clinica. The subject list should be saved as a plain-text file with one subject ID per line (for example, `adni_subs.txt`) and referenced by the Clinica helper scripts in Step 4.
